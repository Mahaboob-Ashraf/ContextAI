...
if (geminiAPIKeys.length === 0) {
    ...
    geminiAPIKeys.forEach((key, index) => {
        try {
            ...
            console.log(`🤖 Gemini AI #${index + 1} initialized (${key.substring(0, 20)}...${key.substring(key.length - 4)})`);
        } catch (err) {
            console.error(`❌ Failed to initialize API key #${index + 1}:`, err.message);
        }
    });
    
    console.log(`✅ Total active API keys: ${geminiModels.length}/${geminiAPIKeys.length}`);
}
...
function getGeminiModel() {
    ...
    console.log(`   🔄 Using Gemini API #${currentModelIndex + 1} of ${geminiModels.length}`);
    return model;
}
...
async function queueAIRequest(requestFn, priority = 0) {
    return new Promise((resolve, reject) => {
        ...
        aiRequestQueue.sort((a, b) => b.priority - a.priority || a.timestamp - b.timestamp);
        
        console.log(`   📊 Queue: ${aiRequestQueue.length} pending, ${activeAIRequests}/${MAX_CONCURRENT_AI_REQUESTS} active`);
...
async function processAIQueue() {
    ...
    while (aiRequestQueue.length > 0 || activeAIRequests > 0) {
        ...
        while (activeAIRequests >= MAX_CONCURRENT_AI_REQUESTS && aiRequestQueue.length > 0) {
        ...
        request.fn()
            ...
            .catch(error => {
                console.error(`   ❌ Queue request failed: ${error.message.substring(0, 100)}`);
                ...
                if (isRetryable && (!request.retryCount || request.retryCount < 2)) {
                    ...
                    console.log(`   🔄 Requeueing request (retry ${request.retryCount}/2)...`);
                    aiRequestQueue.push(request);
                } else {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (isOverloaded && geminiModels.length > 1) {
                console.log(`   ⚠️ API overloaded, cycling through all ${geminiModels.length} keys...`);
                
                // Try each remaining API key (don't retry the one that just failed)
                const startIndex = currentModelIndex;
                for (let keyAttempt = 1; keyAttempt < geminiModels.length; keyAttempt++) {
                    try {
                        // Move to next key
                        currentModelIndex = (currentModelIndex + 1) % geminiModels.length;
                        const alternateModel = geminiModels[currentModelIndex];
                        console.log(`   🔄 Trying API #${currentModelIndex + 1} of ${geminiModels.length}`);
                        return await fn(alternateModel);
                    } catch (alternateError) {
                        if (keyAttempt === geminiModels.length - 1) {
                            console.log(`   ⚠️ All ${geminiModels.length} API keys tried, will wait and retry...`);
            ...
            console.log(`⚠️ API error (attempt ${attempt}/${maxRetries}), retrying in ${delay/1000}s...`);
...
function canBotRespond(groupId) {
    ...
    if (botResponseCooldowns[groupId]) {
        ...
        if (timeSinceLastResponse < BOT_COOLDOWN_MS) {
            const remainingSeconds = Math.ceil((BOT_COOLDOWN_MS - timeSinceLastResponse) / 1000);
            console.log(`   ⏳ Cooldown active: ${remainingSeconds}s remaining - silently ignoring`);
...
client.on('ready', async () => {
    ...
    console.log(`⏳ Waiting ${waitTime/1000}s for client to fully initialize${isReconnecting ? ' (reconnecting - allowing chat sync time)' : ''}...`);
    ...
    console.log('💡 WhatsApp ready - bot will only respond to NEW mentions after this point');
    if (wasReconnecting) {
        console.log('📊 Chat syncing in progress - first fetch may take longer...');
    }
    ...
    client.on('message', async (message) => {
        ...
        try {
            ...
            console.log(`   Bot enabled: ${botConfig.enabled}, Groups: ${botConfig.groups.length}`);
            if (!botConfig.enabled) return;

            const enabledGroupIds = botConfig.groups.map(g => g.id);
            console.log(`   Checking if ${chat.id._serialized} is in enabled groups...`);
            ...
            const mentionedNumbers = mentionedIds.map(id => String(id).split('@')[0]);
            console.log(`   Mentioned Numbers (normalized): ${mentionedNumbers.join(', ')}`);
            ...
            const isMentioned = mentionedIds.length > 0 || mentionedNumbers.includes(myNumber);
            
            const isReplyToMe = message.hasQuotedMsg && (await message.getQuotedMessage()).from === myId;

            console.log(`   Is mentioned: ${isMentioned}, Is reply to me: ${isReplyToMe}`);
            ...
            console.log(`🤖 Bot triggered in ${chat.name} by ${contact.pushname || contact.name}`);
            ...
            const groupConfig = botConfig.groups.find(g => g.id === chat.id._serialized);
            ...
            if (isReplyToMe && message.hasQuotedMsg) {
                ...
                if (quotedMsg.hasMedia && !imageAnalysis) {
                    imageAnalysis = await analyzeImage(quotedMsg, messageText, personality);
                }
            }
            ...
            const summaryMatch = messageText.match(/summarize\s+(?:the\s+)?(?:last\s+)?(\d+)\s+messages?/i);
            
            if (summaryMatch) {
                const numMessages = parseInt(summaryMatch[1]);
                console.log(`   📊 User requested summary of last ${numMessages} messages`);
                
                // Fetch the requested number of messages
                const messages = await chat.fetchMessages({ limit: numMessages });
                const messagesToSummarize = messages.reverse().map(msg => {
                    const sender = msg.author ? msg.author.split('@')[0] : (msg.fromMe ? 'Bot' : 'User');
...
Provide a clear summary in 3-5 bullet points.`;
                ...
                recordBotResponse(chat.id._serialized);
                await message.reply(`📋 *Summary of last ${numMessages} messages:*\n\n${summary}`);
            ...
            const contextCount = conversationMemory[chatId].filter(msg => !msg.isSummary).length;
            console.log(`   💭 Using ${contextCount} conversation messages (excluding summaries, showing last 30)`);
...
async function cleanupAuthFolder() {
    ...
    try {
        ...
        if (await fsPromises.access(authPath).then(() => true).catch(() => false)) {
            await fsPromises.rm(authPath, { recursive: true, force: true, maxRetries: 3 });
...
app.post('/api/keys/add', (req, res) => {
    ...
    try {
        ...
        console.log(`✅ New API key added: #${geminiModels.length} (${apiKey.substring(0, 20)}...${apiKey.substring(apiKey.length - 4)})`);
        ...
        res.status(500).json({ error: 'Failed to initialize API key: ' + err.message });
    }
});
...
app.get('/api/chats', async (req, res) => {
    ...
    try {
        ...
        while (retries > 0) {
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => 
                    setTimeout(() => reject(new Error('Timeout fetching chats')), fetchTimeout)
                );
                ...
                if (err.message.includes('Evaluation failed') && retries > 0) {
                    console.log(`   ⚠️ WhatsApp not ready, retrying... (${retries} attempts left)`);
        ...
        console.log(`✅ Retrieved ${chats.length} chats in ${Date.now() - startTime}ms`);
        ...
        if (err.message.includes('Timeout') && isReconnecting) {
            console.log('   💡 Tip: WhatsApp is still syncing chats after reconnection. Please retry in a few seconds.');
...
app.post('/api/messages', async (req, res) => {
    ...
    try {
        ...
        if (chat.isGroup) {
            ...
            console.log(`✅ Loaded ${Object.keys(participantMap).length} participant names`);
        }
        
        const textMessages = messages
            .filter(m => m.body && m.body.trim())
            .map(m => {
                const authorNumber = m.author ? m.author.split('@')[0] : (m.from ? m.from.split('@')[0] : 'Unknown');
        ...
        if (textMessages.length === 0 && messages.length > 0) {
            console.log(`   ⚠️ No text messages found (${messages.length} total messages - likely media/deleted/system messages)`);
        }
        ...
        const dateRange = timestamps.length > 0 ? {
            oldest: new Date(Math.min(...timestamps) * 1000).toLocaleDateString(),
            newest: new Date(Math.max(...timestamps) * 1000).toLocaleDateString()
        } : null;
        ...
        if (getGeminiModel() && textMessages.length > 0) {
            try {
                ...
                aiSummary = await queueAIRequest(async () => {
                    ...
                    return await generateAISummary(textMessages, chat.name || chat.id.user, chat.isGroup, analysisDepth);
                }, 0);
...
**💡 Tip**: This might be a new chat or all messages might be outside the analyzed range.`;
        ...
        console.log(`✅ Summary generated - ${textMessages.length} messages, ${Object.keys(participants).length} participants`);
...
async function generateAISummary(messages, chatName, isGroup, analysisDepth = 'moderate') {
    const model = getGeminiModel();
    ...
    console.log(`🔬 Analysis mode: ${analysisDepth.toUpperCase()}${isDeepAnalysis ? ' (with research)' : ''}`);
    ...
    if (messages.length > BATCH_SIZE) {
        console.log(`🔄 Large message set detected (${messages.length} messages) - using batch processing...`);
        ...
        console.log(`📦 Processing ${batches.length} batches of ~${BATCH_SIZE} messages each...`);
        
        const batchSummaries = [];
        for (let i = 0; i < batches.length; i++) {
            console.log(`🤖 Processing batch ${i + 1}/${batches.length}...`);
            
            const batchContext = batches[i].map(m => 
                `${m.authorName}: ${m.body}`
            ).join('\n');
            
            const batchPrompt = `Analyze this batch (${i + 1}/${batches.length}) of WhatsApp messages from "${chatName}":
...
Be concise but capture the essence of this conversation segment.`;
            ...
            try {
                const model = getGeminiModel();
                const result = await retryWithBackoff(async (altModel) => {
                    const useModel = altModel || model;
        ...
        const consolidatedContext = batchSummaries.map(b => 
            `Batch ${b.batch}:\n${b.summary}`
        ).join('\n\n---\n\n');
        
        const participantsNote = participantCount > 20 ? 
        ...
        const contributorSection = participantCount <= 20 ? `
...
IMPORTANT: Each bullet (*) MUST start on its own line.`;
        ...
        try {
            ...
            summaryText = `## Batch Summaries (${messages.length} messages)\n\n` + 
                batchSummaries.map(b => `### Batch ${b.batch}\n${b.summary}`).join('\n\n');
        }
...
- Name: count` : '';
        ...
        if (isDeepAnalysis) {
            ...
            prompt = `You are performing a DEEP ANALYSIS of "${chatName}" - ${messages.length} messages${participantCount > 20 ? ` (${participantCount} participants)` : ''}.
...
- Connect ideas and show relationships`;
            ...
            prompt = `Analyze "${chatName}" - ${messages.length} messages${participantCount > 20 ? ` (${participantCount} participants)` : ''}. Create SHORT bullet summary:
...
IMPORTANT: Put each bullet point on a NEW LINE. Do NOT continue points on same line after asterisk (*).`;
        ...
        try {
            console.log(`🤖 Sending ${messages.length} messages to Gemini AI (${isDeepAnalysis ? 'DEEP' : 'MODERATE'} mode)...`);
            const model = getGeminiModel();
            const result = await retryWithBackoff(async (altModel) => {
                const useModel = altModel || model;
                return await useModel.generateContent(prompt);
            ...
            const response = await result.response;
            summaryText = response.text();
            console.log(`✅ Gemini AI summary generated successfully (${isDeepAnalysis ? 'DEEP' : 'MODERATE'} mode)`);
        } catch (err) {
...
app.post('/api/chat-qa', async (req, res) => {
    ...
    try {
        ...
        const chat = await client.getChatById(chatId);
        const messages = await chat.fetchMessages({ limit: parseInt(messageLimit) });
        ...
        const messageContext = textMessages.map(m => 
            `${m.authorName}: ${m.body}`
        ).join('\n');
        
        const qaPrompt = `WhatsApp chat: "${chat.name || chat.id.user}" (${textMessages.length} messages)
...
app.post('/api/dashboard-qa', async (req, res) => {
    ...
    try {
        ...
        console.log(`❓ Question: ${question}`);
        console.log(`📊 Analyzing ${Object.keys(summaries).length} chat summaries...`);
        
        // Build comprehensive context from all summaries
        const summaryContext = Object.entries(summaries).map(([chatId, summary]) => {
            const chatInfo = [
...
function loadBotConfig() {
    try {
        if (fs.existsSync(BOT_CONFIG_FILE)) {
            ...
            console.log(`📥 Loaded bot config: ${loaded.enabled ? 'ENABLED' : 'DISABLED'} for ${loaded.groups.length} groups`);
            return loaded;
        }
    } catch (err) {
...
async function preloadBotMemory() {
    ...
    for (const groupInfo of botConfig.groups) {
        try {
            ...
            for (const msg of messages.reverse()) {
                ...
                try {
                    ...
                    if (msg.fromMe) {
                        ...
                        senderName = contact.pushname || contact.name || msg.author.split('@')[0];
                    }
            ...
            console.log(`   ✅ Loaded ${conversationMemory[chatId].length} messages for ${chat.name}`);
            
            // Small delay to avoid rate limiting
            await new Promise(resolve => setTimeout(resolve, 500));
            
        } catch (err) {
            console.error(`   ❌ Error pre-loading memory for ${groupInfo.name}:`, err.message);
        }
    }
...
app.post('/api/bot/config', async (req, res) => {
    ...
    console.log(`🤖 Bot config updated: ${botConfig.enabled ? 'ENABLED' : 'DISABLED'} for ${botConfig.groups.length} groups`);
    if (botConfig.enabled && botConfig.groups.length > 0) {
        console.log(`   Groups: ${botConfig.groups.map(g => `${g.name} (${g.personality})`).join(', ')}`);
    }
    
    // Pre-load memory if bot was just enabled or new groups added
    if (botConfig.enabled && (!wasEnabled || botConfig.groups.some(g => !oldGroupIds.includes(g.id)))) {
    ...
    try {
        ...
        for (const groupInfo of botConfig.groups) {
            try {
                ...
                for (const message of messages.reverse()) { // Process newest first
                ...
                processedMessages.add(messageId);
                console.log(`🤖 [POLL] Bot triggered in ${chat.name} - message: ${messageId}`);
                ...
                const model = getGeminiModel();
                if (!model) {
                    await mostRecentMention.reply('⚠️ AI not configured. Please set GEMINI_API_KEY.');
                    continue;
                }
                ...
                console.log(`   💭 Using ${conversationMemory[chatId].length} messages from memory (showing last 30)`);
...
Respond naturally - prioritize being helpful. If they ask about previous messages or "last chat", refer to the conversation history above.`;
                ...
                console.warn(`⚠️ Error checking messages in ${groupInfo.name}:`, err.message);
            }
        }
        ...
        if (processedMessages.size > 200) {
            console.log(`🧹 Cleaning up processedMessages Set (size: ${processedMessages.size})`);
